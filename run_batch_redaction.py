...
class ReferenceRedactor:
    ...
    def process(self) -> tuple[bool, str]:
        """
        Executes the full workflow for this file and returns a status tuple.
        """
    ...
    def _calculate_final_coordinates(self):
        """Calculates the final coordinates using the core algorithm."""
    ...
    def _draw_rects_and_save(self) -> tuple[bool, str]:
        """Draws rectangles on the PDF and saves the file."""
        try:
    ...
    # --- Core Algorithm Methods ("Iron Law") ---

    def _extract_raw_reference_boxes(self) -> dict:
        """Parses a GROBID XML and returns raw reference boxes grouped by page."""
        try:
    ...
    def _get_page_dimensions(self) -> dict:
        """Parses the XML to get accurate page dimensions from the <surface> tags."""
        try:
            tree = ET.parse(self.xml_path)
            root = tree.getroot()
            ns = {'tei': 'http://www.tei-c.org/ns/1.0'}
    ...
    def _clean_cluster(self, cluster_boxes: list, page_height: float) -> list:
        """Applies a hybrid strategy to clean a column cluster."""
    ...
    def _cluster_and_merge_boxes(self, boxes: list, page_width: float, page_height: float) -> list:
        """Clusters, cleans, and merges raw GROBID boxes for a single page."""
        if not boxes: return []
...
class BatchRedactionManager:
    ...
    def run(self):
        """Executes the full, two-phase workflow."""
        ...
        # --- Phase 1: Generate XML files from PDFs ---
        self._generate_xmls_concurrently(pdf_files_to_process)

        # --- Phase 2: Redact PDFs using generated XMLs ---
    ...
    def _generate_xmls_concurrently(self, pdf_files: list):
        """Phase 1: Concurrently process PDF files to generate raw XML."""
    ...
    def _get_raw_xml_from_grobid(pdf_path: str, output_xml_path: str, grobid_url: str) -> tuple[bool, str]:
        """Static method to call GROBID. Can be called by concurrent workers."""
        try:
    ...
    def _redact_pdfs_concurrently(self, xml_files: list):
        """Phase 2: Concurrently redact PDFs using the generated XML files."""
    ...
    def _process_single_redaction(self, xml_filename: str) -> tuple[bool, str]:
        """Helper method to instantiate and run ReferenceRedactor for one file."""
